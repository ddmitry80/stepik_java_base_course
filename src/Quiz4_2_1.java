...
public class Quiz4_2_1 {
    ...
    public static void moveRobot(RobotConnectionManager robotConnectionManager, int toX, int toY) {
        boolean flag=false;
        RobotConnectionException err = null;
        for (int i=0; i<3; i++) {
            try (RobotConnection rc = robotConnectionManager.getConnection()) {
                rc.moveRobotTo(toX, toY);
                flag = true;
                //break; // так приводит к невыходу из цикла при ошибке в close(), и провалу теста
                i=3;
            } catch (RobotConnectionException e) {  err=e;  }
        }
        if (!flag)
            throw  err;
        //rc.close();
 ...
 */
    ...
    static class RobotConnectionManager {
        RobotConnection getConnection(){
            System.out.println("attempt number: " + Quiz4_2_1.getI() + ". start RCM");
            if ((Test.getTest(0) instanceof Throwable) && (Quiz4_2_1.getI() == 1)){
    ...
    static class RobotUse implements RobotConnection {
        ...
        public void moveRobotTo(int x, int y){
            System.out.println("attempt number: " + Quiz4_2_1.getI() + ". start RC");
            if ((Test.getTest(1) instanceof Throwable) && (Quiz4_2_1.getI() == 1)){
        ...
        @Override
        public void close(){
            System.out.println("attempt number: " + Quiz4_2_1.getI() + ". start close");
            if ((Test.getTest(2) instanceof Throwable) && (Quiz4_2_1.getI() == 1)){
    ...
    static class Test {
        ...
        static RuntimeException getTest(int args){
            ...
            return testArray[Quiz4_2_1.getRepeatNumber()][args];
        }
    }
}